...
pub fn calc_tint(rgb: &str, tint: &f64) -> String {
    ...
    let moved = if tint < 0.0 {
        luminance as f64 * (1.0 + tint)
    } else {
        luminance as f64 * (1.0 - tint) + EXCEL_HLS_MAX as f64 * tint
    };
    convert_excel_hls_to_rgb(hue, (moved as i32).clamp(0, EXCEL_HLS_MAX), saturation)
}
...
#[cfg(test)]
mod tests {
    use super::{calc_tint, normalize_excel_tint, EXCEL_TINT_SHORT_MAX};
